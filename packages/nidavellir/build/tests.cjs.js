...
var flowEntry = _interopDefault(require('rollup-plugin-flow-entry'));
...
mocha.describe('Testing Key derivation', function () {
  deriveFixtures.private.forEach(test => {
    mocha.it(`Deriving private key without public key ${test[0]} for index ${test[2]}`, async function () {
      ...
      chai.assert.deepEqual(childKey, expectedChildKey);
    });
  });
  deriveFixtures.privateWithPublic.forEach(test => {
    mocha.it(`Deriving private key with public key ${test[0]} for index ${test[2]}`, async function () {
      const privateKey = test[0];
      const chainCode = test[1];
      const index = test[2];
      const hardened = test[3];
  ...
  deriveFixtures.public.forEach(test => {
    mocha.it(`Deriving public key ${test[0]} for index ${test[2]}`, async function () {
...
const network = 'main';
mocha.describe(`Testing Extended Key functions`, function () {
  XKeyFixtures.string.forEach(test => {
    ...
    mocha.it(`Extended key from string: ${test[0]}`, function () {
      const resultedXkey = fromString(base58Key, network);
      chai.assert.deepEqual(resultedXkey, xkey);
    });
    mocha.it(`Extended key to string: ${test[1]}`, function () {
      if (!xkey.publicKey) xkey = _objectSpread({}, xkey, {
        publicKey: `02${test[1]}`
      });
      const resultedBase58Key = toString(xkey, network);
      chai.assert.equal(resultedBase58Key, base58Key);
    });
  });
});
...
mocha.describe('Testing HD Key', function () {
  HDKeyFixtures.fromSeed.forEach(test => {
    mocha.it(`Creating HD key from phrase ${test[0]}`, async function () {
      const childKey = await fromSeed$1(test[1]);
      const xkey = toString$1(childKey);
      chai.assert.deepEqual(xkey, test[2]);
    });
  });
  HDKeyFixtures.fromString.forEach(test => {
    mocha.it(`Creating HD key from xkey ${test[0]} with path ${test[1]}`, function () {
      const opts = {};
      if (test[1]) opts.path = test[1].split('/');
      const hdKey$$1 = fromString$1(test[0], opts);
      chai.assert.equal(hdKey$$1.hardened, test[2]);
      chai.assert.equal(hdKey$$1.path.join('/'), test[3]);
    });
  });
  HDKeyFixtures.fromStringErrors.forEach(test => {
    mocha.it(`Error on Creating HD key with error type ${test[2]}`, function () {
      try {
        fromString$1(test[0], {
          path: test[1].split('/')
        });
      } catch (e) {
        chai.assert.equal(e.message, test[2]);
      }
    });
  });
  HDKeyFixtures.fromPath.forEach(test => {
    mocha.it(`Deriving HD key from xkey ${test[0]} with path ${test[1]}`, async function () {
      ...
      while (path.length) {
        ...
        chai.assert.equal(xprivkey, test[testIndex][0], 'xpriv');
        chai.assert.equal(xpubkey, test[testIndex][1], 'xpub');
        testIndex++;
      }
    });
  });
...
mocha.describe('Testing Bip44 HD Key path derivation', function () {
  fixtures$1.createPaths.forEach(test => {
    mocha.it(`Deriving HD key ${test[0]} from HD Settings ${test[3]}`, async function () {
      const parentKey = fromString$1(test[0]);
      let account = 0;
      let coinType = 0;
      ...
      for (const hdPath of hdPaths) {
        ...
        chai.assert.equal(xprivkey, test[testIndex++], 'xpriv');
        chai.assert.equal(xpubkey, test[testIndex++], 'xpub');
      }
    });
  });
});
...
mocha.describe('Testing HD Key', function () {
  KeyPairFixtures.fromWif.forEach(test => {
    mocha.it(`Creating Key Pair from WIF ${test[0]}`, async function () {
      const keyPair$$1 = await fromWif(test[0], network$3);
      chai.assert.equal(keyPair$$1.publicKey, test[1]);
    });
  });
});
...
const hash = sha512Hmac(key, data);
chai.assert.equal(hash, res);

for (const fixture in baseFixtures) {
  mocha.describe(`Testing base ${fixture}`, function () {
    const fixtureBase = base[fixture].check;
    baseFixtures[fixture].valid.forEach(({
      string,
      payload
    }) => {
      mocha.it(`Decode valid string: ${string}`, async function () {
        const resultedPaylod = fixtureBase.decode(string);
        chai.assert.equal(resultedPaylod, payload);
      });
      mocha.it(`Encode valid payload: ${payload}`, async function () {
        const resultedString = fixtureBase.encode(payload);
        chai.assert.equal(resultedString, string);
      });
    });
    baseFixtures[fixture].invalid.forEach(({
      ...
      mocha.it(`Test invalid string: ${string}`, async function () {
        try {
          fixtureBase.decode(string);
        } catch (e) {
          chai.assert.equal(e.message, exception);
        }
      });
    });
  });
...
const createSave = (expected, delay, cbs) => {
  ...
  return async cacheData => {
    if (cbs.stop) return;
    cbs.e = new Error();
    chai.assert.notEqual(cacheData, expected);
    chai.assert.deepEqual(cacheData, expected);
    saveAttempts++;
    ...
    if (delay) {
      ...
      chai.assert.isAtLeast(saveTime, minDelay);
    }
...
for (const test in fixtures$3.persister) {
  ...
  mocha.describe(`Testing persister for ${test} with delay of ${delay || 100}ms`, function () {
    this.timeout(0);
    mocha.before('Test using cache as a function', async function () {
      cbs.stop = false;
      await persistedCache({});
      await persistedCache(cache);
    });
    sets.map(([param, value, saveTime], i) => {
      mocha.it(`Set '${param}' to ${value}, after ${saveTime} milliseconds`, function (done) {
        ...
        setTimeout(() => {
          ...
          child[key] = value;
          chai.assert.equal(expectedChild[key], child[key]);
          done(cbs.e);
        }, interval);
      });
    });
    mocha.after('Stop the caceh using cache as a function', async function () {
      cbs.stop = true;
