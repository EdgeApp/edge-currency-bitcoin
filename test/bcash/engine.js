/* global describe it */
const BcashCurrencyPluginFactory = require('../../lib/index.test.js').BcashCurrencyPluginFactory
...
const WALLET_TYPE = 'wallet:bcash44'
...
describe(`Engine Creation Errors for Wallet type ${WALLET_TYPE}`, function () {
  ...
  before('Plugin', function () {
    return BcashCurrencyPluginFactory.makePlugin(opts).then((BcashPlugin) => {
      assert.equal(BcashPlugin.currencyInfo.currencyCode, 'BCH')
      plugin = BcashPlugin
      keys = plugin.createPrivateKey(WALLET_TYPE)
      keys = plugin.derivePublicKey({type: WALLET_TYPE, keys: {bcashKey: keys.bcashKey}})
    })
  })
...
describe(`Start Engine for Wallet type ${WALLET_TYPE}`, function () {
  before('Create local cache file', function (done) {
    ...
    .then(() => BcashCurrencyPluginFactory.makePlugin(opts))
    .then((BcashPlugin) => {
      assert.equal(BcashPlugin.currencyInfo.currencyCode, 'BCH')
      plugin = BcashPlugin
      keys = plugin.createPrivateKey(WALLET_TYPE)
...
describe(`Is Address Used for Wallet type ${WALLET_TYPE}`, function () {
  ...
  it('Checking a non empty address from cache', function (done) {
    assert.equal(engine.isAddressUsed('125NGZ4Hdn4mkuns6gtx1yZwcgnWsr4Lio'), true)
    done()
  })
