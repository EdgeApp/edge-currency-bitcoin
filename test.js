...
var logger = new bcoin.logger({ level: 'debug', console: true });
  ...
  var chain = new bcoin.chain({
    db: 'leveldb',
    // location: process.env.PWD + "/db" + '/spvchain1496768800031' + new Date().getTime(),
    // location: process.env.PWD + "/db" + '/spvchain1496768800031',
  ...
  logger.writeConsole = function(level, module, args) {
    console.log("UNO", level,module,args);
  ...
  logger.info = function(level, module, args) {
    console.log("DUOS", level,module,args);
  };

var walletdb = new bcoin.walletdb({ db: 'leveldb', location: process.env.PWD + "/db" + '/wallet1' });
